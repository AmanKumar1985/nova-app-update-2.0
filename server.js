...
function saveBase64Media(dataStr, category = 'images') {
  ...
  try {
    ...
    else if (mime.includes('audio') || mime.includes('ogg') || mime.includes('mp3') || mime.includes('wav')) ext = 'mp3';
    ...
    console.error('Failed to save base64 media:', err);
    return dataStr;
  }
}
...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS blocks (blocker_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE, blocked_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE, PRIMARY KEY(blocker_id, blocked_id));
  CREATE TABLE IF NOT EXISTS password_resets (email TEXT PRIMARY KEY, code TEXT NOT NULL, expires_at INTEGER NOT NULL);
  CREATE TABLE IF NOT EXISTS follow_requests (id TEXT PRIMARY KEY, from_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE, to_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE, created_at INTEGER NOT NULL, UNIQUE(from_id, to_id));
  CREATE TABLE IF NOT EXISTS rate_limits (key TEXT NOT NULL, endpoint TEXT NOT NULL, hits INTEGER NOT NULL DEFAULT 0, window_end INTEGER NOT NULL, PRIMARY KEY(key, endpoint));
`);
...
addColumnIfMissing('users', 'is_private', 'INTEGER NOT NULL DEFAULT 0');
...
addColumnIfMissing('posts', 'images', 'TEXT');
...
const validImage = value => (/^data:image\/(png|jpe?g|webp|gif);base64,/i.test(value) || /^\/uploads\//i.test(value) || /^https?:\/\/.+/i.test(value)) && Buffer.byteLength(value, 'utf8') <= 12 * 1024 * 1024;
const validVideo = value => (/^data:video\/(mp4|webm|quicktime|x-m4v);base64,/i.test(value) || /^\/uploads\//i.test(value) || /^https?:\/\/.+/i.test(value)) && Buffer.byteLength(value, 'utf8') <= 25 * 1024 * 1024;
...
function publicUser(row, viewerId) {
  ...
  const isBlocked = viewerId ? !!db.prepare('SELECT 1 FROM blocks WHERE (blocker_id=? AND blocked_id=?) OR (blocker_id=? AND blocked_id=?)').get(viewerId, row.id, row.id, viewerId) : false;
  const isFollowing = viewerId ? !!db.prepare('SELECT 1 FROM follows WHERE follower_id=? AND following_id=?').get(viewerId, row.id) : false;
  const isFollowPending = (viewerId && !isFollowing) ? !!db.prepare('SELECT 1 FROM follow_requests WHERE from_id=? AND to_id=?').get(viewerId, row.id) : false;
  return { 
    id: row.id, 
...
function rateLimit(limit, windowMs) {
  return (req, res, next) => {
    ...
    if (Math.random() < 0.05) db.prepare('DELETE FROM rate_limits WHERE window_end < ?').run(current);
    let record = db.prepare('SELECT * FROM rate_limits WHERE key=? AND endpoint=?').get(key, endpoint);
    if (!record) {
      db.prepare('INSERT OR IGNORE INTO rate_limits (key, endpoint, hits, window_end) VALUES (?,?,1,?)').run(key, endpoint, current + windowMs);
      return next();
    }
    if (record.window_end < current) {
      db.prepare('UPDATE rate_limits SET hits=1, window_end=? WHERE key=? AND endpoint=?').run(current + windowMs, key, endpoint);
      return next();
    }
    if (record.hits >= limit) return res.status(429).json({ error: 'Too many requests. Please try again shortly.' });
    db.prepare('UPDATE rate_limits SET hits=hits+1 WHERE key=? AND endpoint=?').run(key, endpoint);
    next();
  };
}
...
app.post('/api/auth/forgot-password', rateLimit(5, 60000), (req, res) => {
  ...
  db.prepare('INSERT OR REPLACE INTO password_resets (email, code, expires_at) VALUES (?,?,?)').run(email, code, expiresAt);
  console.log(`[Nova Password Reset Code] OTP for ${email} is: ${code}`);
  res.json({ ok: true, message: `Verification code generated: ${code}`, code });
});
app.post('/api/auth/reset-password', rateLimit(5, 60000), (req, res) => {
  const email = clean(req.body.email).toLowerCase();
  const code = clean(req.body.code);
  const newPassword = clean(req.body.newPassword);
  if (!validEmail(email)) return res.status(400).json({ error: 'Invalid email.' });
  if (!code) return res.status(400).json({ error: 'Enter the 6-digit verification code.' });
  if (!validPassword(newPassword)) return res.status(400).json({ error: 'Password must be 5-72 characters.' });
...
app.patch('/api/me', auth, (req, res) => { 
  ...
  if (avatar) avatar = saveBase64Media(avatar, 'images');
  db.prepare('UPDATE users SET display_name=?, bio=?, avatar=? WHERE id=?').run((displayName||req.user.display_name).slice(0,50),bio.slice(0,180),avatar || req.user.avatar || null,req.user.id); 
  res.json({user:publicUser(db.prepare('SELECT * FROM users WHERE id=?').get(req.user.id),req.user.id)}); 
});
app.patch('/api/me/privacy', auth, (req, res) => {
  const isPrivate = req.body.isPrivate ? 1 : 0;
  db.prepare('UPDATE users SET is_private=? WHERE id=?').run(isPrivate, req.user.id);
  res.json({ user: publicUser(db.prepare('SELECT * FROM users WHERE id=?').get(req.user.id), req.user.id) });
});
...
app.get('/api/users', auth, (req,res) => { 
  ...
  if (q.startsWith('#')) {
    const tag = q.replace(/^#/, '');
    const rows = db.prepare("SELECT * FROM posts WHERE caption LIKE ? ORDER BY created_at DESC LIMIT 30").all(`%#${tag}%`);
    return res.json({ users: [], posts: rows.map(p => postDto(p, req.user.id)), tag });
  }
  const rows=q ? db.prepare("SELECT * FROM users WHERE id != ? AND (username LIKE ? OR lower(display_name) LIKE ?) LIMIT 30").all(req.user.id,`%${q}%`,`%${q}%`) : []; 
  res.json({users:rows.map(row=>publicUser(row,req.user.id))}); 
});
...
app.post('/api/users/:username/follow', auth, (req,res) => {
  ...
  if(isFollowing) {
    // Unfollow — also remove any leftover request
    db.prepare('DELETE FROM follows WHERE follower_id=? AND following_id=?').run(req.user.id, target.id);
    db.prepare('DELETE FROM follow_requests WHERE from_id=? AND to_id=?').run(req.user.id, target.id);
  } else if(target.is_private) {
    // Private account: toggle follow request
    const pending = db.prepare('SELECT 1 FROM follow_requests WHERE from_id=? AND to_id=?').get(req.user.id, target.id);
    if(pending) {
      db.prepare('DELETE FROM follow_requests WHERE from_id=? AND to_id=?').run(req.user.id, target.id);
    } else {
      db.prepare('INSERT OR IGNORE INTO follow_requests (id,from_id,to_id,created_at) VALUES (?,?,?,?)').run(id(), req.user.id, target.id, now());
      notify(target.id, req.user.id, 'follow_request');
    }
  } else {
...
function postDto(post, viewerId) { 
  const author=db.prepare('SELECT * FROM users WHERE id=?').get(post.author_id); 
  let imagesArr = [];
  if (post.images) {
    try { imagesArr = JSON.parse(post.images); } catch(e){}
  }
  if (!imagesArr.length && post.image) {
    imagesArr = [post.image];
  }
  return {
    id:post.id,
    image:post.image,
    images:imagesArr,
    isReel:!!post.is_reel,
    isInstagram:!!post.is_instagram,
    platform:post.platform||(post.is_instagram?'instagram':'nova'),
    externalUrl:post.external_url||'',
    filter:post.filter||'normal',
    caption:post.caption,
    createdAt:post.created_at,
    author:publicUser(author,viewerId),
    likes:db.prepare('SELECT COUNT(*) count FROM likes WHERE post_id=?').get(post.id).count,
    liked:!!db.prepare('SELECT 1 FROM likes WHERE post_id=? AND user_id=?').get(post.id,viewerId),
    saved:!!db.prepare('SELECT 1 FROM saved_posts WHERE post_id=? AND user_id=?').get(post.id,viewerId),
    comments:db.prepare('SELECT c.id,c.text,c.created_at,u.username,u.display_name FROM comments c JOIN users u ON u.id=c.user_id WHERE c.post_id=? ORDER BY c.created_at DESC LIMIT 3').all(post.id).reverse().map(c=>({id:c.id,text:c.text,createdAt:c.created_at,user:{username:c.username,displayName:c.display_name}}))
  }; 
}
app.get('/api/posts', auth, (req,res) => { const posts=db.prepare("SELECT * FROM posts WHERE COALESCE(is_instagram,0)=0 AND COALESCE(platform,'')!='instagram' ORDER BY created_at DESC LIMIT 100").all(); res.json({posts:posts.map(p=>postDto(p,req.user.id))}); });
app.get('/api/reels', auth, (req,res) => { const posts=db.prepare("SELECT * FROM posts WHERE is_reel=1 AND COALESCE(is_instagram,0)=0 AND COALESCE(platform,'')!='instagram' ORDER BY created_at DESC LIMIT 100").all(); res.json({posts:posts.map(p=>postDto(p,req.user.id))}); });
app.post('/api/posts', auth, rateLimit(20, 60000), (req,res) => { 
  let imageRaw = String(req.body.image||''); 
  let imagesListRaw = Array.isArray(req.body.images) ? req.body.images : [];
  const isReel = req.body.isReel ? 1 : 0;
  
  if (imagesListRaw.length > 0) {
    imagesListRaw = imagesListRaw.map(img => saveBase64Media(img, 'images'));
    imageRaw = imagesListRaw[0];
  } else if (imageRaw) {
    imageRaw = saveBase64Media(imageRaw, isReel ? 'videos' : 'images');
    imagesListRaw = [imageRaw];
  }
  
  const caption=clean(req.body.caption); 
  const filter=clean(req.body.filter||'normal'); 
  if(!imageRaw) return res.status(400).json({error: isReel ? 'Please choose an MP4 or WEBM video.' : 'Please choose a valid image.'}); 
  
  const post={id:id(),author_id:req.user.id,image:imageRaw,images:JSON.stringify(imagesListRaw),caption:caption.slice(0,1000),created_at:now(),is_reel:isReel,filter}; 
  db.prepare('INSERT INTO posts (id,author_id,image,images,caption,created_at,is_reel,filter) VALUES (?,?,?,?,?,?,?,?)').run(post.id,post.author_id,post.image,post.images,post.caption,post.created_at,post.is_reel,post.filter); 
  res.status(201).json({post:postDto(post,req.user.id)}); 
});
...
app.get('/api/youtube/shorts', auth, async (req, res) => {
  ...
  if (!apiKey) {
    return res.json({
      shorts: shuffleArray(fallbackShorts),
      nextPageToken: ''
    });
  }

  try {
    const defaultQueries = ['shorts viral trending', 'funny indian shorts', 'music viral shorts', 'dance shorts 2026', 'gaming shorts hindi', 'motivational shorts hindi', 'tech shorts viral'];
    const randomQuery = defaultQueries[Math.floor(Math.random() * defaultQueries.length)];
    const rawQ = clean(req.query.q || randomQuery);
    const qStr = rawQ.includes('shorts') ? rawQ : `${rawQ} shorts`;
    const searchUrl = `https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&videoDuration=short&regionCode=IN&relevanceLanguage=hi&maxResults=20&q=${encodeURIComponent(qStr)}&pageToken=${encodeURIComponent(pageToken)}&key=${apiKey}`;
    const searchRes = await fetch(searchUrl);

    if (!searchRes.ok) {
      const errData = await searchRes.json().catch(() => ({}));
      console.warn('YouTube Shorts API warning:', errData.error ? errData.error.message : 'API key limit');
      return res.json({ shorts: shuffleArray(fallbackShorts), nextPageToken: '' });
    }
    ...
    const finalShorts = shorts.length ? (req.query.fresh === '1' ? shuffleArray(shorts) : shorts) : shuffleArray(fallbackShorts);
    ...
    res.json({ shorts: shuffleArray(fallbackShorts), nextPageToken: '' });
  }
});
...
app.post('/api/stories', auth, rateLimit(10,60000), (req,res) => { 
  let image=String(req.body.image||''); 
  if(!validImage(image)) return res.status(400).json({error:'Please choose a valid image.'}); 
  image = saveBase64Media(image, 'images');
  const story={id:id(),author_id:req.user.id,image,created_at:now()}; 
  db.prepare('INSERT INTO stories (id,author_id,image,created_at) VALUES (?,?,?,?)').run(story.id,story.author_id,story.image,story.created_at); 
...
app.post('/api/messages/:username', auth, rateLimit(120, 60000), (req,res) => {
  ...
  let audio=req.body.audio ? String(req.body.audio) : null;
  if (img) img = saveBase64Media(img, 'images');
  if (audio) audio = saveBase64Media(audio, 'audio');
...
// ── ICE servers for WebRTC (Problem 8: TURN support) ──────────────────────────
...
// ── Follow requests (Problem 3: private accounts) ─────────────────────────────
app.get('/api/me/follow-requests', auth, (req, res) => {
  const rows = db.prepare(
    'SELECT fr.id, fr.created_at, u.username, u.display_name, u.avatar ' +
    'FROM follow_requests fr JOIN users u ON u.id=fr.from_id ' +
    'WHERE fr.to_id=? ORDER BY fr.created_at DESC'
  ).all(req.user.id);
  res.json({ requests: rows.map(r => ({ id: r.id, createdAt: r.created_at, user: { username: r.username, displayName: r.display_name, avatar: r.avatar } })) });
});
app.post('/api/follow-requests/:id/accept', auth, (req, res) => {
  const fr = db.prepare('SELECT * FROM follow_requests WHERE id=? AND to_id=?').get(req.params.id, req.user.id);
  if (!fr) return res.status(404).json({ error: 'Request not found.' });
  db.prepare('INSERT OR IGNORE INTO follows (follower_id, following_id) VALUES (?,?)').run(fr.from_id, fr.to_id);
  db.prepare('DELETE FROM follow_requests WHERE id=?').run(fr.id);
  notify(fr.from_id, req.user.id, 'follow_accept');
  res.json({ ok: true });
});
...
// ── Google OAuth (Problem 2) ───────────────────────────────────────────────────
function getGoogleRedirectUri(req) {
  if (process.env.GOOGLE_REDIRECT_URI && process.env.GOOGLE_REDIRECT_URI.trim()) {
    return process.env.GOOGLE_REDIRECT_URI.trim();
  }
  const host = req.get('host') || '';
  const proto = (host.includes('localhost') || host.includes('127.0.0.1')) ? (req.protocol || 'http') : 'https';
  return `${proto}://${host}/api/auth/google/callback`;
}

app.get('/api/auth/google', (req, res) => {
  const clientId = process.env.GOOGLE_CLIENT_ID;
  if (!clientId) return res.status(503).send('<h2>Google login not configured. Add GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET to .env</h2>');
...
app.get('/api/auth/google/callback', async (req, res) => {
  ...
  try {
    ...
    if (!tokenData.access_token) {
      ...
      const detailErr = tokenData.error_description || tokenData.error || 'token_failed';
      return res.redirect('/?auth_error=' + encodeURIComponent(detailErr));
    }
    const userRes = await fetch('https://www.googleapis.com/oauth2/v3/userinfo', {
      headers: { Authorization: `Bearer ${tokenData.access_token}` }
    });
    const googleUser = await userRes.json();
    if (!googleUser.email) return res.redirect('/?auth_error=no_email');
    let user = db.prepare('SELECT * FROM users WHERE lower(email)=?').get(googleUser.email.toLowerCase());
    if (!user) {
      const base = (googleUser.email.split('@')[0]).replace(/[^a-z0-9_.]/gi, '').toLowerCase().slice(0, 18) || 'user';
      let username = base; let attempt = 0;
      while (db.prepare('SELECT 1 FROM users WHERE username=?').get(username)) { attempt++; username = `${base}${attempt}`; }
      const uid = id();
      db.prepare('INSERT INTO users (id,username,display_name,email,avatar,created_at) VALUES (?,?,?,?,?,?)').run(uid, username, (googleUser.name || username).slice(0, 50), googleUser.email.toLowerCase(), googleUser.picture || null, now());
      user = db.prepare('SELECT * FROM users WHERE id=?').get(uid);
    }
    const token = createSession(user.id);
    res.redirect(`/?nova_token=${encodeURIComponent(token)}`);
  } catch (err) {
...
function seedReelsIfEmpty() {
  try {
    // Delete any previous Instagram seed data from DB
...
// ── Story auto-cleanup every hour (Problem 14) ────────────────────────────────
function cleanupExpiredStories() {
  const cutoff = now() - 24 * 3600000;
  db.prepare('DELETE FROM story_views WHERE story_id IN (SELECT id FROM stories WHERE created_at < ?)').run(cutoff);
  db.prepare('DELETE FROM stories WHERE created_at < ?').run(cutoff);
}
